class PlantSpacesController < ApplicationController
  before_action :set_plant_space, only: [:show, :edit, :update]

  def new
    @plant_space = PlantSpace.new
  end

  def create
    @plant_space = PlantSpace.new(plant_space_params)
    @plant_space.house_id = current_user.houses(id: params[:id])
    if @plant_space.save
      redirect_to houses_path
    else
      flash[:alert] = "Couldn't save that space"
    end
  end

  private

  def set_plant_space
    @plant_space = House.plant_space.find_by(id: params[:id])
  end

  def plant_space_params
    params.require(:plant_space).permit(:name, :sunlight_rating, :temp, :humidity, :house_id, :plant_id)
  end
end